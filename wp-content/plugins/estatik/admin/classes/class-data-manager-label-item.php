...
class Es_Data_Manager_Label_Item extends Es_Data_Manager_Term_Item
{
	...
	public static function save()
	{
		...
		if ( self::is_valid_ajax() && ! empty( $_REQUEST[ $nonce ] ) && wp_verify_nonce( $_REQUEST[ $nonce ], $nonce ) ) {
					...
					<?php foreach ( $colors as $key => $color ) : ?>
						<input type="radio"
							<?php checked( $term_color, $color ); ?>
							   ...
							   class="es-radio-color es-radio-color-<?php echo str_replace('#', '', $color); ?>"
	...
	{
		$color = update_term_meta( $_POST['id'], 'es_color', $_POST['color'] );
